...
function RestaurantCard({id,name,avgRating,cloudinaryImageId,costForTwo,cuisines,sla,aggregatedDiscountInfoV3}) {
  ...
  const display=aggregatedDiscountInfoV3?.header?"":"hidden";
      ...
      <div className={`${darkModeClass}`}>
       ...
       <div className="flex justify-between py-3 text-center">
        <h1 className={"text-white font-semibold text-[0.725rem] w-[3rem] "+RatingColor}><StarIcon fontSize="small"/>{avgRating}</h1>
         <h1 className="text-[0.725rem]">{sla?.slaString}</h1>
         <h1 className="text-[0.725rem]">{costForTwo}</h1>
       </div>
       <div className="border-b w-full my-2"></div>
       <h1 className={"text-xs "+darkForOffer+" "+display}><LocalOfferIcon fontSize='small' className='pr-1'/>{aggregatedDiscountInfoV3?.header+" "+aggregatedDiscountInfoV3?.subHeader}</h1>
