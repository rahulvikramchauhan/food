...
function RestaurantList() {
  ...
  const getRestaurantData = async () => {
    ...
    dispatch(addRestaurant(jsonData))
  };
  ...
  function filterTheRestaurant(searchValue,restaurantList)
  {
     ...
     const result=restaurantList.filter(function(restaurant){
      
        return restaurant?.info?.cuisines.join("").toLowerCase().includes(searchValue.toLowerCase());
     } )
  ...
  if(filteredRestaurant.length===0)
  {
    ...
    return (
      <div className={"mx-[4rem] mt-5 "+darkModeClass}>
        ...
        <div className={"flex flex-wrap mt-[4rem] mb-[6.5rem] "+darkModeClass}>
